package be.planty.assistant.web.websocket;
...
public class MessagingService {
    ...
    public void onRequest( //@Payload final ActionRequest requestPayload,
        ...
        @DestinationVariable final String emailAddress
    ) {
        ...
        logger.debug(
            "On requestPayload from '" +
            skillSessionId +
            "' (" +
            skillUsername.orElse(null) +
            ")" +
            " for '" +
            emailAddress +
            "' : " +
        ...
        final Optional<String> username = filteredAgents.stream().findFirst().map(a -> a.getUser().getLogin());
        ...
        try {
            prettyPayload =
                payload instanceof String
                    ? (String) payload
                    : (payload instanceof byte[] ? new String((byte[]) payload) : objectWriter.writeValueAsString(payload));
