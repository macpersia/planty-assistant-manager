...
import { EntityArrayResponseType, PairingRequestService } from '../service/pairing-request.service';
...
export class PairingRequestComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPairingRequest): number => this.pairingRequestService.getPairingRequestIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.pairingRequests = this.refineData(dataFromBody);
  }

  protected refineData(data: IPairingRequest[]): IPairingRequest[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IPairingRequest[] | null): IPairingRequest[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.pairingRequestService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
